...
The core function. Called by the extension every 30 minutes and by pg_cron for all users.
...
CREATE TABLE notification_queue (
  ...
  type TEXT NOT NULL CHECK (type IN ('daily_reminder', 'badge_earned', 'friend_request')),
  title TEXT NOT NULL,
  body TEXT NOT NULL,
...
Each user gets a unique friend code derived from their ID:
```
friend_code = base36(hash(user_id)).slice(0, 8).toUpperCase()
Format: XXXX-XXXX
```

Stored as a generated column or computed on read.
